...
export const getStatusLabel = (status: string): string => {
  const labels: Record<string, string> = {
    ...
    unpaid: 'Non payé',
    paused: 'En pause',
    no_record: 'Aucun abonnement trouvé',
  };
...
export const getProductByPriceId = (priceId: string): Product | undefined => {
  return PRODUCTS_MAP[priceId];
};

export const getProductsByType = (type: 'plan' | 'addon', profile?: 'individual' | 'agency'): Product[] => {
  return PRODUCTS_DATA.filter(product => {
    if (product.type !== type) return false;
    if (profile && product.profile !== profile) return false;
    return true;
  });
};

export const calculatePrice = (profile: 'individual' | 'agency', cadence: 'month' | 'quarter' | 'year'): number => {
  const priceId = getPlanPriceId(profile, cadence);
  const product = PRODUCTS_MAP[priceId];
...
export const calculateAddonPrice = (
  ...
  if (type === 'dept') {
    priceId = getDeptAddonPriceId(profile, cadence);
  } else {
    priceId = getUserAddonPriceId(cadence);
  }
...
export const calculateTTC = (priceHT: number, vatRate: number = 0.20): number => {
  return priceHT * (1 + vatRate);
};

export const getCadenceLabel = (cadence: 'month' | 'quarter' | 'year'): string => {
  const labels = {
    month: 'Mensuel',
    quarter: 'Trimestriel',
    year: 'Annuel'
...
export const getCadenceDiscount = (cadence: 'month' | 'quarter' | 'year'): number => {
  // Calculate discount percentage compared to monthly pricing
  switch (cadence) {
    case 'quarter':
